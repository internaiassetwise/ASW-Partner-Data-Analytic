    ...
    <MapContainer center={[13.7563, 100.5018]} zoom={11} style={{ height: "100%", width: "100%" }} scrollWheelZoom zoomControl={false}>
        ...
        >
          <Popup>
            <div style={{ minWidth: "160px" }}>
              <strong style={{ fontSize: "13px", color: NAVY }}>{f.properties.name}</strong>
              <br />
      ...
      <MarkerClusterGroup iconCreateFunction={createClusterIcon} showCoverageOnHover={false} maxClusterRadius={50}>
        {partners.features.map((f, i) => {
          const color = TYPE_COLORS[f.properties.entity_type] || "#666";
          return (
            <CircleMarker
              key={`p-${i}`}
              center={[f.geometry.coordinates[1], f.geometry.coordinates[0]]}
            ...
            >
              <Popup>
                <div style={{ minWidth: "150px" }}>
                  <strong style={{ fontSize: "12px" }}>{f.properties.name}</strong>
                  <br />
                  <span style={{ fontSize: "11px", color: "#888" }}>
                    {TYPE_LABELS[f.properties.entity_type] || f.properties.entity_type}
    ...
    <div style={{ position: "absolute", top: "10px", left: "50px", zIndex: 1000 }}>
      <button
        ...
        style={{
          ...
          alignItems: "center", gap: "4px", color: "#4b5563",
        }}
      >
        มุมมองทั้งหมด
      </button>
